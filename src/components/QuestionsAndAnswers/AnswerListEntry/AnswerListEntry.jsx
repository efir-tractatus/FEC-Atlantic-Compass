  ...
  return (
    <div className="answer-list-entry Q-grid-container">
      ...
      <div className="answer-utility-container Q-col-2">
        ...
        <InteractionTracker
          widget="QandA"
          element="Mark-answer-helpful"
          render={({ postInteraction }) => (
            <a
              className="answer-utility-link"
              id="answer-helpful"
              onClick={(e) => {
                if (!isMarkedHelpful) {
...
var highlightSeller = (name) => {
  if (name.toLowerCase() === 'seller') {
    return (
      <p className="answer-utility-text">
        by <b>{name}</b> ,
      </p>
    );
  } else {
    return <p className="answer-utility-text">by {name} ,</p>;
  }
};

var postHelpfulness = (answerId, productId, populateQuestions) => {
  axios
    .put(`/catwalk/qa/answers/helpful/${answerId}`)
    .then((response) => {
...
var postReported = (answerId, productId, populateQuestions) => {
  axios
    .put(`/catwalk/qa/answers/report/${answerId}`)
    .then((response) => {
      console.log('success', response);
    })
    .catch((err) => {
